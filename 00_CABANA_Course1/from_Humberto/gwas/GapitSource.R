...
{ 
  ...
  #print(length(rsquare_base))
	PWI.Filtered=cbind(GWAS[,-5],rsquare_base,rsquare)
	colnames(PWI.Filtered)=c("SNP","Chromosome","Position ","P.value", "maf", "nobs", "Rsquare.of.Model.without.SNP","Rsquare.of.Model.with.SNP")
...
function(model_store=NULL,myG=NULL,myGD=NULL,myGM=NULL,myKI=NULL,myY=NULL,myCV=NULL,rel=NULL,h2=NULL,NQTN=NULL
  )
{ 
...
GAPIT_list=list(group.from=group.from ,group.to=group.to,group.by=group.by,DPP=100000,kinship.cluster=kinship.cluster, kinship.group=kinship.group,kinship.algorithm=kinship.algorithm,                                                    
                ...
                sangwich.top=sangwich.top,sangwich.bottom=sangwich.bottom,QC=QC,GTindex=NULL,LD=LD,plot.bin=10^5,file.output=file.output,cutOff=cutOff, Model.selection = Model.selection,output.numerical = output.numerical,
