...
########
#This script implements a fast approximation named
#HCoreA
########
...
entangle<-function(xodd,xeven){a<-c(NA);length(a)<-2*length(xodd);a[c(seq(1,length(a),2),seq(2,length(a),2))]<-c(xodd,xeven);a}
...
##########
#The following script implements an approximations
#to maximize KL distance starting from the 21th accession
#It is expected that, as the number of selected accessions grows
#the approximation gets better.

ApproxMaxDeltaR<-function(data,cset){
	#Define which accessions of data to add to cset
	accessions<-names(data)
	accessions<-accessions[!is.element(accessions,cset)]	
	#Obtain the average caused by each accession
	my.q<-rowMeans(data[,cset],na.rm=T) #Odd frequencies
	my.q<-entangle(my.q,1-my.q)  #Add Even frequencies
	a<-c(NA)
	length(a)<-length(accessions)
	for(i in 1:length(accessions)){
		a[i]<-divergence(data[,accessions[i]],my.q)
		}
	#Accession that contributed most
	accessions[a==max(a)][1]
}
...
HCoreA<-function(data,n){
	...
	if(n>30){
	for(i in 31:n){
		selected<-ApproxMaxDeltaR(data,core)
		core<-c(core,selected)
		print(i)}
		}
...
###############
#Do not use HalfOdd
#dat is date with firs two columns as markers and alleles
#Socre biallelic
s.biallelic<-function(data){
	data<-HalfOdd(data[-c(1,2)])
	scoreBiallelic(data)
}
#Get core
h.coreA<-function(data,n){
	data<-HalfOdd(data[-c(1,2)])
	HCoreA(data,n)
}

###############
#Easy functions with file saving
###############
s.biallelic.file<-function(data){
	x<-s.biallelic(data) #List of objects for specificity, rareness and divergence
	TabSpec<-data.frame("marker"=data[,1],"allele"=data[,2],"specificity"=x$specificity)
	write.csv(TabSpec, file="SpecificityTable.csv", quote=F, row.names=F)
	write.csv(x$table, file="PopulationTable.csv", quote=F, row.names=F)
	u<-hist(x$specificity,plot=F)
	xlim<-c(0,floor(range(x$specificity)[2])+1)
	ylim<-c(0,max(u$counts)+.1*max(u$counts))
	jpeg("HistSpec.jpg", width=600, height=600, pointsize=18)
	hist(x$specificity, col="green",main="Histogram of specificity", xlab="specificity", xlim=xlim, ylim=ylim)
	dev.off()
	jpeg("HistRar.jpg",width=600, height=600, pointsize=18)
	hist(x$rareness, col="green",main="Histogram of rareness",xlab="rareness")
	dev.off()
	jpeg("HistDiv.jpg",width=600, height=600, pointsize=18)
	hist(x$divergence, col="green", main="Histogram of divergence",xlab="rareness")
	dev.off()
	}
	
	
	h.coreA.file<-function(data, n, filename){
		core<-h.coreA(data,n)
		core<-data.frame("Population"=core)
		write.csv(core, file=filename, row.names=F, quote=F)
	}
